...
# 중요!! import game을 수행하면 game 디렉터리의 모듈 또는 game 디렉터리의 __init__.py에 정의한 것만 참조할 수 있다.

# 또 다음처럼 echo_test 함수를 사용하는 것도 불가능하다.
# import game.sound.echo.echo_test
# 중요!! 도트 연산자(.)를 사용해서 import a.b.c처럼 import할 때 가장 마지막 항목인 c는 반드시 모듈 또는 패키지여야만 한다.
...
echo.echo_test()
"""
# NameError: name 'echo' is not defined 오류 발생
# 분명 game.sound 패키지에서 모든 것(*)을 import하였으므로 echo 모듈을 사용할 수 있어야 할 것 같은데 echo라는 이름이 정의되지 않았다는 이름 오류(NameError)가 발생했다.
# 중요!! 이렇게 특정 디렉터리의 모듈을 *를 사용하여 import할 때에는 해당 디렉터리의 __init__.py 파일에 __all__ 변수를 설정하고 import할 수 있는 모듈을 정의해 주어야 한다.
...
# 중요!! 이렇게 __all__과 상관없이 무조건 import되는 경우는 from a.b.c import * 에서 from의 마지막 항목인 c가 모듈인 경우이다.
...
"""
# render.py에 아래 내용 작성 (from game.sound.echo import echo_test 문장을 추가하여 echo_test 함수를 사용할 수 있도록 수정)
from game.sound.echo import echo_test
def render_test():
    print ("render")
    echo_test()
...
def render_test():
    ...
    echo_test()
"""
# from game.sound.echo import echo_test가 from ..sound.echo import echo_test로 변경되었다. 여기에서 ..은 부모 디렉터리를 의미한다.
