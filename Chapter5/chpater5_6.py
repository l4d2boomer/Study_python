...
# 그러기 위해 어떤 라이브러리가 존재하고 어떻게 사용하는지 알아야 한다. 자주 사용되고 꼭 알아 두면 좋은 라이브러리를 중심으로 하나씩 살펴보자.
...
print(os.environ['PATH'])  # 'C:\\ProgramData\\Oracle\\Java\\javapath;...생략...'
...
# time.time
# time.time()은 UTC를 사용하여 현재 시간을 실수 형태로 돌려주는 함수이다. 1970년 1월 1일 0시 0분 0초를 기준으로 지난 시간을 초 단위로 돌려준다.
import time
print(time.time())  # 988458015.73417199
...
import time
print(time.strftime('%x', time.localtime(time.time())))  # '05/01/01'
print(time.strftime('%c', time.localtime(time.time())))  # '05/01/01 17:22:21'
...
# 중요!! 하지만 앞에서 설명했듯이 스레드를 사용하면 5초의 시간이 걸리는 long_task 함수를 동시에 실행할 수 있으니 시간을 줄일 수 있다.
